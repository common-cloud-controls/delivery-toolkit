...
func publishArtifact(token, websiteRepo, catalogPath, tag, artifactType string, mdContent, yamlContent []byte) error {
	mdDest := fmt.Sprintf("src/content/catalogs/%s/%s/%s.md", artifactType, catalogPath, tag)
	yamlDest := fmt.Sprintf("public/data/catalogs/%s/%s/%s.yaml", artifactType, catalogPath, tag)
	commitMsg := fmt.Sprintf("release: %s %s@%s", catalogPath, artifactType, tag)

	title := extractTitle(string(mdContent))
	pagePath := fmt.Sprintf("/catalogs/%s/%s/%s", artifactType, catalogPath, tag)
